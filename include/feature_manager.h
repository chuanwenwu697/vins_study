...
{
public:
  FeaturePerFrame(const Eigen::Matrix<double, 7, 1> &_point, double td)//每个路标点在一张图片上的信息
  {
    point.x() = _point(0);
    point.y() = _point(1);
    point.z() = _point(2);
...
public:
  ...
  vector<FeaturePerFrame> feature_per_frame;//每个路标点由多个连续的图像观测到
  ...
  int endFrame();//返回最后一个观测到这个特征点的图像ID
};
