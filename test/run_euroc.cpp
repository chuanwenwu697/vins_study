...
string sData_path = "/home/chuanwen/ws/vins_mono_course/VINS-Course/data/MH_05/mav0/";
...
void PubImageData()
{
	...
	{
		//cout << "sImage_line: " << sImage_line << endl;
		std::istringstream ssImuData(sImage_line);//通过sImage_line对ssImuData进行初始化(sImage_line 1403638518077829376 1403638518077829376.png)		
		/*string s;  
		while(ssImuData>>s)  
		{  
			cout<<s<<endl;  
    	} */
		ssImuData >> dStampNSec >> sImgFileName;;//dStampNSec:时间戳　sImgFileName：图像文件名
		...
		// cv::waitKey(0);
		usleep(50000*nDelayTimes);//usleep函数能把线程挂起一段时间， 单位是微秒（千分之一毫秒）
	}
	fsImage.close();
}
