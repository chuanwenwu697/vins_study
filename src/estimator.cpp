...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {   ////从第二帧开始，将imu数据放入到预积分函数中。pre_integrations代表了滑动窗口中的关键帧
        pre_integrations[frame_count]->push_back(dt, linear_acceleration, angular_velocity);
        //if(solver_flag != NON_LINEAR)tmp_pre_integrations代表了每一帧（包含关键帧和非关键帧）
        ...
        //中值积分
        int j = frame_count;
        Vector3d un_acc_0 = Rs[j] * (acc_0 - Bas[j]) - g;//起始时刻加速度
        Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - Bgs[j];//角速度
        Rs[j] *= Utility::deltaQ(un_gyr * dt).toRotationMatrix();//deltaQ:转换成四元数
        Vector3d un_acc_1 = Rs[j] * (linear_acceleration - Bas[j]) - g;//终止时刻加速度
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);//加速度
        Ps[j] += dt * Vs[j] + 0.5 * dt * dt * un_acc;//位置
        Vs[j] += dt * un_acc;//速度
    }
    acc_0 = linear_acceleration;
    gyr_0 = angular_velocity;
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))//一、1)将image中的特征点信息加入feature。2)关键帧的选择（计算视差）
    ...
    ImageFrame imageframe(image, header);//将图像的特征点信息、时间戳、imu预积分放入到ImageFrame类中
    imageframe.pre_integration = tmp_pre_integration;
    all_image_frame.insert(make_pair(header, imageframe));//二、包含一帧图像的所有信息，包括特征点，R,T,IMU预积分，是否是关键帧
    tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};

    if (ESTIMATE_EXTRINSIC == 2)//２：说明不知道外参　　１：说明外参有一个初始的估计，但是不准确　　０：说明有一个准确的外参
    {
        ...
        if (frame_count != 0)
        {
            ...
            Matrix3d calib_ric;
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))//三、外参的标定：计算imu和视觉的外参
            {
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if (ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1)
            {
                ...
                result = initialStructure();//四、视觉惯导联合初始化
                initial_timestamp = header;
            }
            if (result)
            {
                solver_flag = NON_LINEAR;
                solveOdometry();//五、后端非线性优化和边缘化
                slideWindow();//六、滑动窗口，移除边缘化的帧
...
bool Estimator::initialStructure()
{
    ...
    {   //通过计算线加速度的标准差，判断IMU是否有充分运动激励，以进行初始化
        ...
        if (var < 0.25)
        {
            cout << "IMU excitation not enouth!" << endl;
    ...
    //二、构SFMFeature对象：遍历滑动窗口里面的所有特征点，将feature里面的信息放入到SFMFeature结构体中
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        {
            imu_j++;//观测特征点的起始帧（特征点连续出现在之后的几帧imu_j++）
            Vector3d pts_j = it_per_frame.point;//3D特征点坐标
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()}));//观测到该特征点的图像帧ID和归一化平面坐标
        }
    ...
    Vector3d relative_T;
    int l;
    //三、在滑动窗口中找到第Ｌ帧（当作世界坐标系），并计算当前帧（第10帧）到Ｌ帧的Ｒ，Ｔ
    ...
    //四、对窗口中的每一关键帧求SFM
    /*
        １）三角化出同时被参考帧L（假设第５帧）和第10帧观测的特征点１
        ２）通过已知深度特征点１和在第６帧上的投影，pnp求解出第6帧的位姿
        ３）已知第６帧的位姿，三角化出同时被第6帧和第10帧的特征点２
        ４）分别将6,7,8,9帧和第L帧三角化，以三角化出更多的特征点
        ５）通过已知特征点６（第５步三角化求得）和再第4帧的投影,pnp求解第４帧的位姿。已知了第4帧的位姿，三角化出同时被第４帧和第Ｌ帧观测到的特征点８
        循环第５步
        ６）还没有三角化的特征点，只要超过两个观察帧，则通通三角化出来
        ７）构建BA模型，优化关键帧的位姿和特征点的位置

        已经求出all_image_frame（滑动窗口）中关键帧的位姿，接下来求解滑动窗口中非关键帧的位姿

        １）对于所有的图像帧all_image_frame,包括关键帧和非关键帧，提供初始的R,T估计，然后用solvePnP进行优化，得到每一帧的姿态
            　对于all_image_frame中的关键帧，因为位姿已知，直接赋值
            　对于all_image_frame中的非关键帧，赋值附近关键帧的位姿，然后用pnp求解出其位姿
        ２）对于all_image_frame中的所有帧，将其旋转从相机坐标系转换到IMU坐标系
    */
    ...
    for (int i = 0; frame_it != all_image_frame.end(); frame_it++)
    {
        ...
        if ((frame_it->first) == Headers[i])//（时间戳等于Headers时，为关键帧）当为关键帧的时候，位姿已知，直接赋值，然后将其旋转从相机坐标系转化到IMU坐标系
        {
        ...
        Matrix3d R_inital = (Q[i].inverse()).toRotationMatrix();//对于非关键帧，赋值附近关键帧的位姿，然后利用已知位置的3D点，pnp求解出其位姿
        ...
        cv::cv2eigen(t, T_pnp);
        T_pnp = R_pnp * (-T_pnp);
        frame_it->second.R = R_pnp * RIC[0].transpose();//将非关键帧的旋转从相机坐标系转换到IMU坐标系
...
//视觉初始对齐（图像与IMU对齐）
/**
 * @brief 该函数主要实现了陀螺仪的偏置校准(加速度偏置没有处理)，计算速度V[0:n]、重力g、尺度s。
 * 
 * @return true 
 * @return false 
 */
bool Estimator::visualInitialAlign()
{
    ...
    for (int i = 0; i <= frame_count; i++)//将all_image_frame中的关键帧位姿取出，并标记为关键帧
    {
    ...
    */ 
    for (int i = frame_count; i >= 0; i--)//得到了以第Ｌ帧坐标系为世界坐标系，在IMU坐标系中各个关键帧到第0帧的相对位移
    ...
    for (auto &it_per_id : f_manager.feature)//将特征点的深度恢复正常尺度
    {
    ...
    // 6. 通过将重力旋转到z轴上，得到世界坐标系与相机坐标系c0之间的旋转矩阵rot_diff
    Matrix3d R0 = Utility::g2R(g);//R0是指第Ｌ帧坐标系下的重力矢量与（0,0,9.81）的旋转偏差
    double yaw = Utility::R2ypr(R0 * Rs[0]).x();
    R0 = Utility::ypr2R(Eigen::Vector3d{-yaw, 0, 0}) * R0;//目的是保证第一帧的偏行角为0
    g = R0 * g;
    //Matrix3d rot_diff = R0 * Rs[0].transpose();
    Matrix3d rot_diff = R0;
    // 7. 所有变量从参考坐标系 c 0 c_0 c0​旋转到世界坐标系w
    for (int i = 0; i <= frame_count; i++)//通过第Ｌ帧，将位置，旋转速度与世界坐标系对齐
    {
        Ps[i] = rot_diff * Ps[i];
        Rs[i] = rot_diff * Rs[i];
        Vs[i] = rot_diff * Vs[i];
...
/**
 * @brief 遍历滑动窗口中的所有关键帧,返回滑动窗口中第一个满足视差的帧，为l帧，以及R,T，可以三角化
 * @Description 判断每帧到窗口最后一帧对应特征点的平均视差是否大于30
                solveRelativeRT()通过基础矩阵计算当前帧与第l帧之间的R和T,并判断内点数目是否足够
 * 
 * @param relative_R 当前帧到第l帧之间的旋转矩阵R
 * @param relative_T 当前帧到第l帧之间的平移向量T
 * @param l 从第一帧开始到滑动窗口中第一个满足视差足够的帧，这里的l帧之后作为参考帧做全局SFM用
 * @return true 可以进行初始化
 * @return false 不满足初始化
 */
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        if (corres.size() > 20)//1.匹配的特征点数目足够
        {   //2.计算的平均视差大于30       
            ...
            //判断是否满足初始化条件：视差>30且内点数目满足要求
            //同时返回窗口最后一帧（当前帧）到第l帧（参考帧）的R,T
...
{
    TicToc t_margin;
    if (marginalization_flag == MARGIN_OLD)//次新帧是关键帧，移除最旧帧和惯性数据边缘化掉
    {
...
{
    sum_of_back++;//统计一共多少次merg滑窗第一帧的情况
    ...
    if (shift_depth)
    {
        ...
        f_manager.removeBack();//最老帧删除后，其相应的特征点
}
