...
/**
 * @brief 五点法求相机初始位姿，通过本质矩阵得到R，t
 *         findFundamentalMat()采用RANSAC算法求解本质矩阵E
 *         recoverPose()通过本质矩阵得到Rt
 * 
 * @param corres      第Ｌ帧和最后一帧的共视特征点集合
...
bool MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    if (corres.size() >= 15)
    {
         ...
         *      double  param1 = 0.3,            点到对极线的最大距离，超过这个值的点将被舍弃
        ...
        */
        ...
        Rotation = R.transpose();           //第一帧坐标系到第二帧坐标系的旋转矩阵 转换成　第二帧坐标系到第一帧坐标系的旋转矩阵
