...
{
    //norm返回的是向量的二范数(模长)
    //清楚了norm()的定义后，normalize()其实就是把自身的各元素除以它的范数
...
{
    int all_frame_count = all_image_frame.size();
    int n_state = all_frame_count * 3 + 3 + 1;//速度、重力向量（在第0帧Camera相机坐标系下的重力向量）、尺度
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++, i++)
    {
        ...
        MatrixXd tmp_A(6, 10);//位置和速度增量（６）、两帧的速度、重力、尺度（10）
        tmp_A.setZero();
        VectorXd tmp_b(6);
        tmp_b.setZero();
