...
double INIT_DEPTH;
double MIN_PARALLAX;            //关键帧选择阈值（像素）
double ACC_N, ACC_W;            //加速度计噪声、随机游走
double GYR_N, GYR_W;            //陀螺仪噪声、随机游走

vector<Eigen::Matrix3d> RIC;    //相机、imu外参（旋转）
vector<Eigen::Vector3d> TIC;    //相机、imu外参（平移）

Eigen::Vector3d G{0.0, 0.0, 9.8};//重力加速度

double BIAS_ACC_THRESHOLD;
double BIAS_GYR_THRESHOLD;
double SOLVER_TIME;             //最大迭代时间（ms），以保证实时性
int NUM_ITERATIONS;             //最大迭代次数，以保证实时性
int ESTIMATE_EXTRINSIC;         //外参估计（imu和相机　0：精确的外参，１：有初始外参，２：没有外参）
int ESTIMATE_TD;
int ROLLING_SHUTTER;            //0:全局快门相机，1:滚动快门相机
string EX_CALIB_RESULT_PATH;    //外参结果文件路径
string VINS_RESULT_PATH;
// string IMU_TOPIC;
double ROW, COL;                //行、列值
double TD, TR;                  //时间偏移的初始值TD
...
int MAX_CNT;                    //特征追踪中最大特征数量
int MIN_DIST;                   //特征点之间最小距离
// int WINDOW_SIZE;
int FREQ;                       //追踪结果发布频率
double F_THRESHOLD;             //ransac阈值（像素）
int SHOW_TRACK;                 //将跟踪图像发布为主题(1)
bool STEREO_TRACK;              //是否为双目跟踪
int EQUALIZE;                   //如果图像太暗或太亮，请尝试均衡以找到足够的特征
int FISHEYE;                    //如果用鱼眼，就用它。将加载一个圆形遮罩，以去除边缘噪声点
bool PUB_THIS_FRAME;
