...
void System::PubImageData(double dStampSec, Mat &img)
{
    if (!init_feature)
    {
        ...
        init_feature = 1;//第一幅影像
        return;
    }

    if (first_image_flag)//第二幅影像
    {
    ...
    last_image_time = dStampSec;
    // frequency control
    if (round(1.0 * pub_count / (dStampSec - first_image_time)) <= FREQ)//round()函数仅仅保留到整数位，即仅仅对小数点后一位四舍五入
    {
        PUB_THIS_FRAME = true;
        // reset the frequency control
        if (abs(1.0 * pub_count / (dStampSec - first_image_time) - FREQ) < 0.01 * FREQ)//时间间隔内的发布频率十分接近设定频率时，更新时间间隔起始时刻，并将数据发布次数置0
        {
            first_image_time = dStampSec;
            pub_count = 0;
        }
    ...
    {
        pub_count++;
        cout << "pub_count:" << pub_count<< "        dStampSec:" << setprecision(19) <<dStampSec <<endl;
        shared_ptr<IMG_MSG> feature_points(new IMG_MSG());
        feature_points->header = dStampSec;
        vector<set<int>> hash_ids(NUM_OF_CAM);//set是一种容器
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            //}
            // skip the first image; since no optical speed on frist image(跳过第一张图片；因为第一张图像上没有速度)
            if (!init_pub)
            {
...
void System::ProcessBackEnd()
{
    ...
    while (bStart_backend)
    {
        ...
        //          cout << " img_sec: " <<std::setprecision(19)<<  measurements[i].second.get()->header << endl;           
        //          cout << " feature_buf size: " <<  feature_buf.size() << endl;
        //          cout << "imu_size: " << measurements[i].first.size() << endl;
        //          for(int j=0;j<measurements[i].first.size();j++){
        //              cout << "第"<< j << "个imu的时间戳"<<std::setprecision(19)<<measurements[i].first[j].get()->header
        //              <<measurements[i].first[j]->linear_acceleration.transpose()<< "   "<<
        //              measurements[i].first[j]->angular_velocity.transpose()<<endl;
        //          }
        //     }
        ...
        lk.unlock();
        m_estimator.lock();
        //cout << "measurements numbers:"<<measurements.size() << endl;
        for (auto &measurement : measurements)
        {
            ...
            /*Matrix<double, 7, 1>是一个7维向量，依次存放着当前 feature_id 的特征点在 camera_id 的相机中的归一化坐标，像素坐标 和 像素运动速度
            *    第一个int表示特征点的索引feature_id
            ×    第二个int表示特征点对应的相机索引 camera_id
            */
            map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> image;
            
            /*cout << "measurements.iamge" << endl;
            cout << "img_sec：　"<< img_msg->header<< endl;
            cout << "measurements.iamge->points.size"<< img_msg->id_of_point.size()<< endl;
            cout << img_msg->points.size()<<endl;*/
            /*for(int i=0;i<img_msg->points.size();i++)
            {
                cout << "points"<<i<<" : "<< img_msg->points[i].transpose() << endl;
                cout << "id_of_point"<<i<<" : "<< img_msg->id_of_point[i] << endl;
                cout << "u_of_point"<<i<<" : "<< img_msg->u_of_point[i] << endl;
                cout << "v_of_point"<<i<<" : "<< img_msg->v_of_point[i] << endl;
                cout << "velocity_x_of_point"<<i<<" : "<< img_msg->velocity_x_of_point[i] << endl;
                cout << "velocity_y_of_point"<<i<<" : "<< img_msg->velocity_y_of_point[i] << endl;
            }*/
            ...
            for (unsigned int i = 0; i < img_msg->points.size(); i++) 
            {
                ...
                int camera_id = v % NUM_OF_CAM;//一直为0
